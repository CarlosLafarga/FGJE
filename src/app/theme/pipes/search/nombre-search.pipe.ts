      ...
      if (defaultFilter) {
        return funcionarios.filter( item =>
            filterKeys.reduce(( x, keyName ) =>
                (x && new RegExp( searchNombre[keyName], 'gi' ).test( this.removeAccents( item[keyName] ) )) || searchNombre[keyName] == "", true));
      }
      else {
        return funcionarios.filter( it => {
          return filterKeys.some(( key ) => {
            return new RegExp( searchNombre[key], 'gi' ).test( this.removeAccents( it[key] ) ) || searchNombre[key] == "";
          });
        });
      }
    }
