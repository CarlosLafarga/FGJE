      ...
      if (defaultFilter) {
        return funcionarios.filter( item =>
            filterKeys.reduce(( x, keyName ) =>
                (x && new RegExp( searchApMaterno[keyName], 'gi' ).test( this.removeAccents(item[keyName])  )) || searchApMaterno[keyName] == "", true));
      }
      else {
        return funcionarios.filter( it => {
          return filterKeys.some(( key ) => {
            return new RegExp( searchApMaterno[key], 'gi' ).test( this.removeAccents(it[key])  ) || searchApMaterno[key] == "";
          });
        });
      }
    }
